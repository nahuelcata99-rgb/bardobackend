...
function getUserInfoFromMetadata(metadata, paymentPayer) {
  ...
  // Intentar obtener de la metadata primero
  ...
  // Usar información de MercadoPago como fallback
  if (paymentPayer) {
...
function createTicketsWithUserInfo(ticketsCount, userInfo, metadata) {
  return Array(ticketsCount).fill(0).map((_, index) => ({
    ...
    // Información de identificación del storage
...
router.post('/create-preference', async (req, res) => {
  try {
    ...
    const preference = {
      ...
      payer: payer,
      back_urls: {
        success: `${process.env.FRONTEND_URL || 'http://localhost:8100'}/pago-exitoso`,
        failure: `${process.env.FRONTEND_URL || 'http://localhost:8100'}/pago-error`,
        pending: `${process.env.FRONTEND_URL || 'http://localhost:8100'}/pago-pendiente`
      },
...
// POST /api/mercadopago/direct-reservation - Crear reserva directa
...
router.get('/order/:orderId', async (req, res) => {
  try {
    ...
    const payments = searchResponse.body.results;
    const reservation = await Reservation.findOne({ orderId: orderId });
